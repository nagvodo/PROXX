...
namespace PROXX
{
    public class View
    {
        ...
        {
            for (int y = Game.Length - 1; y >= -2; y--)
            {
                for (int x = -2; x < Game.Length; x++)
                {
                    if (y == -2)
                    {
                        if (x >= 0)
                    ...
                    switch (cell.Visibility)
                    {
                        case Cell.CellVisibility.Open:
                            ...
                            {
                                if (cell.AdjacentBlackHolesCount > 0)
                                    ForegroundColor = ConsoleColor.DarkBlue;

                                Write(cell.AdjacentBlackHolesCount);
                            }
        ...
        public int DrawEnterCoordinates(string axis)
        {
            while (true)
            {
                ...
                var userInput = ReadLine();
                var isValidInt = int.TryParse(userInput, out int i);
                if (!isValidInt || i < 0 || i >= Game.Length)
                {
                    WriteLine("It must be an integer between 0 and {0}.\r\nThe value you entered (\'{1}\') is invalid", Game.Length - 1, userInput);
                    continue;
                }
                WriteLine();
                return i;
